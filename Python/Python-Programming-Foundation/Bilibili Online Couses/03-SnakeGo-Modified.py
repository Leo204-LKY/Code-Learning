...
def your_score(score):                                    # 定义分数
    ...
    dis.blit(value, [snake_block, snake_block])
...
def gameLoop():          # 构建一个循环函数用于蛇的捕食过程
    ...
    # 设置食物位置
    foodx = round(random.randrange(snake_block, dis_width - 2 * snake_block) / 10.0) * 10.0
    foody = round(random.randrange(snake_block, dis_height - 2 * snake_block) / 10.0) * 10.0
    # round 代表四舍五入取整
    ...
    while not game_over:
        ...
        while game_close == True:
            dis.fill(red)
            pygame.draw.rect(dis, blue, [snake_block, snake_block, dis_width - 2 * snake_block, dis_height - 2 * snake_block])
            for x in snake_list:
                pygame.draw.rect(dis, black, [x[0], x[1], snake_block, snake_block])
            message("You Lose! Press R to play again or Esc to quit.", red)
